...
from utils.loss import Segloss,BoundaryLoss,JointLoss
...
def validation(model, criterion, val_dataloader, device):
    ...
    running_metrics_val_edge = runningScore(2)
    ...
    with torch.no_grad():
        ...
        for i_val, (valid_img, valid_mask, valid_edge) in enumerate(val_loader):

            valid_img = valid_img.to(device, dtype=torch.float32)
            valid_mask = valid_mask.to(device, dtype=torch.float32)
            valid_edge = valid_edge.to(device, dtype=torch.float32)

            valid_mask_pred, valid_edge_pred = model(valid_img)
            val_loss = criterion(valid_mask_pred, valid_mask, valid_edge_pred, valid_edge)

            mask_pred = (torch.sigmoid(valid_mask_pred) > 0.5).float()
            mask_pred = mask_pred.squeeze().cpu().numpy()
            mask_gt = valid_mask.squeeze().cpu().numpy()

            edge_pred = (torch.sigmoid(valid_edge_pred) > 0.5).float()
            edge_pred = edge_pred.squeeze().cpu().numpy()
            edge_gt = valid_edge.squeeze().cpu().numpy()


            running_metrics_val.update(mask_gt, mask_pred)
            running_metrics_val_edge.update(edge_gt, edge_pred)
            val_loss_meter.update(val_loss.item())

            val_loader.set_postfix({"Validation Loss":'{:.4f}'.format(val_loss_meter.avg),
                                   "Mask_MIoU":'{:.4f}'.format(running_metrics_val.get_scores()[0]['Mean_IoU']),
                                   "Edge_MIoU":'{:.4f}'.format(running_metrics_val_edge.get_scores()[0]['Mean_IoU'])})


    return running_metrics_val, running_metrics_val_edge, val_loss_meter


def train(args,
          img_path, mask_path, val_img_path, val_mask_path,
          model, epochs, batch_size, lr, loss_fun, dir_checkpoint, logger,
          resume='',save_cp=True):
    # Setup seeds
    ...
    # Setup datasets
    train_datasets = Mydatasets(img_path, mask_path, train_transform, label_transform, with_edge=True, aug=True)
    val_datasets = Mydatasets(val_img_path, val_mask_path, train_transform, label_transform, with_edge=True, aug=False)
    ...
    # loss Function
    if loss_fun == 'jointloss':
        criterion = JointLoss(w=args.w,alpha=args.alpha)
    else:
        raise NotImplementedError
    logger.info(f"Criterion paramter:w:{criterion.w},alpha:{criterion.alpha}")
    ...
    for epoch in range(start_epoch,epochs):
        ...
        model.train()
        for i_iter,(train_img, train_mask, train_edge) in enumerate(train_dataloader):
            ...
            train_edge = train_edge.to(device, dtype=torch.float32)
            ...
            # forward
            train_mask_pred, train_edge_pred = model(train_img)
            # loss
            loss = criterion(train_mask_pred, train_mask, train_edge_pred, train_edge)
            ...
            if (globe_step + 1) % 420 == 0:
                running_metrics_val, running_metrics_val_edge, val_loss_meter = \
                    validation(model, criterion, val_dataloader, device)
                ...
                edge_score, edge_class_iou = running_metrics_val_edge.get_scores()

                metirc_list = ['Overall_Acc', 'P_IoU', 'Mean_IoU']
                for j in metirc_list:
                    logger.info("{}: {}".format(j, edge_score[j]))
                    # writer.add_scalar("edge_val_metrics/{}".format(j), edge_score[j], globe_step + 1)
                    writer.add_scalar("edge_val_metrics/{}".format(j), edge_score[j], epoch)

                for k, v in edge_class_iou.items():
                    logger.info("{}: {}".format(k, v))
                    # writer.add_scalar("val_metrics/cls_{}".format(k), v, globe_step + 1)
                    writer.add_scalar("edge_val_metrics/cls_{}".format(k), v, epoch)
...
def  get_args():
    ...
    # losstype
    parser.add_argument('-losstype', '--loss-type',type=str, default=r'jointloss',
                        help='the type of loss ',dest = 'losstype')
    parser.add_argument('-w', '--w',type=int, default=1, help='the weight of segloss ',dest = 'w')
    parser.add_argument('-alpha', '--alpha',type=int, default=1, help='the weight of boundary loss',dest = 'alpha')
...
if __name__ == '__main__':
    ...
    try:
        if  args.losstype == 'bceloss' or args.losstype == 'segloss':
            n_classes = 1
        elif args.losstype == 'jointloss':
            n_classes = 1
    except:
    ...
    if args.load:
        model.load_state_dict(
            torch.load(args.load)['model_state']
        )
